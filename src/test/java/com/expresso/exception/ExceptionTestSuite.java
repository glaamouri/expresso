...
public class ExceptionTestSuite {
    ...
    void testSyntaxExceptions() {
        ...
        assertThrows(SyntaxException.class, () -> evaluator.evaluate("sum(1, 2", context));

        // Unbalanced brackets
        assertThrows(SyntaxException.class, () -> evaluator.evaluate("$arr[0", context));

        // Invalid operator sequence
        assertThrows(SyntaxException.class, () -> evaluator.evaluate("5 + * 3", context));
    ...
    void testPropertyNotFoundExceptions() {
        ...
        // Property not found on object - using a custom class to ensure
        // PropertyAccessException
        class TestObject {
        }
        context.setVariable("obj", new TestObject());
        ex = assertThrows(PropertyAccessException.class, () -> evaluator.evaluate("$obj.nonExistentProperty", context));
        assertTrue(ex.getMessage().contains("nonExistentProperty"));

        // Array index out of bounds
        context.setVariable("list", Arrays.asList(1, 2, 3));
        ex = assertThrows(ArrayIndexOutOfBoundsException.class, () -> evaluator.evaluate("$list[10]", context));
        ...
        ex = assertThrows(PropertyNotFoundException.class, () -> evaluator.evaluate("$person.address.city", context));
    }
    ...
    void testEvaluationExceptions() {
        ...
        ex = assertThrows(FunctionExecutionException.class, () -> evaluator.evaluate("requiresTwoArgs(1)", context));
    ...
    void testArithmeticExceptions() {
        ...
        // Division by zero
        Exception ex = assertThrows(ArithmeticExpressionException.class, () -> evaluator.evaluate("5 / 0", context));
        assertTrue(ex.getMessage().contains("Division by zero"));
        assertEquals("/", ((ArithmeticExpressionException) ex).getOperation());

        // Modulo by zero
        ex = assertThrows(ArithmeticExpressionException.class, () -> evaluator.evaluate("5 % 0", context));
    ...
    void testInvalidOperationExceptions() {
        ...
        Exception ex = assertThrows(InvalidOperationException.class, () -> evaluator.evaluate("true + 5", context));
        assertTrue(ex.getMessage().contains("Cannot add these types"));

        // Cannot subtract strings
        ex = assertThrows(InvalidOperationException.class, () -> evaluator.evaluate("'hello' - 'world'", context));
        ...
        ex = assertThrows(InvalidOperationException.class, () -> evaluator.evaluate("'hello' > true", context));
    ...
    void testExceptionHierarchy() {
        ...
        try {
            ...
            // Use equals() comparison instead of exact type comparison since values could
